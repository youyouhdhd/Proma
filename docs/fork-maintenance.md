...
- 本 Fork 的主要产品定制在独立分支 `origin/feat/custom-model-reasoning`，当前提交为 `0a19e264`，尚未合并进 `main`。

因此，Fork 不会因为上游更新而自动获得新功能，但也不需要放弃定制：以后从 `upstream/main` 建立同步分支，再重放下面记录的定制提交即可。当前上游已经明显领先，首次同步前应预留冲突处理和完整构建时间。
